      ...
      <div v-if="orders.length" class="order-list">
        <article v-for="record in orders" :key="record.id" class="order-card" :class="[`is-${record.status}`,{'is-selected':isOrderSelected(record)}]">
          ...
          <div class="order-card__body">
            ...
            <section class="card-info"><div class="card-info__heading"><span class="block-label">收件信息</span><a-button v-if="!aftersales" type="text" size="small" class="card-info__copy" :disabled="!hasShippingInfo(record)" aria-label="复制收件信息" title="复制收件人、手机号和收货地址" @click.stop="copyShippingInfo(record)"><CopyOutlined /> 复制</a-button></div><h4>{{ record.consignee||'未填写' }} <small class="mono">{{ record.phone||'—' }}</small></h4><p :title="record.address">{{ record.address||'未填写收货地址' }}</p></section>
