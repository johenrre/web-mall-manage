      ...
      <div v-if="orders.length" class="order-list">
        <article v-for="record in orders" :key="record.id" class="order-card" :class="`is-${record.status}`">
          ...
          <div class="order-card__body">
            ...
            <section class="card-buyer"><span class="block-label">下单用户</span><h4>{{ record.user?.nickname||record.user?.username||`用户 ${record.user_id}` }}</h4><span v-if="purchaseSummary(record)" class="purchase-badge" :class="{'is-repeat':isRepeatPurchase(record)}">{{ purchaseSummary(record) }}</span></section>
    ...
    <a-drawer v-model:open="detailOpen" title="订单详情" placement="right" :width="900">
      ...
      <div v-if="selected" class="detail-stack">
        ...
        <a-descriptions title="订单与收货" bordered :column="2" size="small">
          <a-descriptions-item label="下单用户"><span class="detail-buyer">{{ selected.user?.nickname||selected.user?.username||`用户 ${selected.user_id}` }}<span v-if="purchaseSummary(selected)" class="purchase-badge" :class="{'is-repeat':isRepeatPurchase(selected)}">{{ purchaseSummary(selected) }}</span></span></a-descriptions-item>
...
.purchase-badge{display:inline-flex;align-items:center;width:max-content;padding:1px 7px;border-radius:99px;color:#60756e;background:#edf3f0;font-size:9px;line-height:16px;white-space:nowrap}.purchase-badge.is-repeat{color:#956328;background:#f7ead7}.detail-buyer{display:flex;flex-wrap:wrap;align-items:center;gap:8px}
...
@media(max-width:760px){.order-card__header,.order-identity{align-items:flex-start}.order-identity{flex-direction:column;gap:2px}.order-card__body{grid-template-columns:1fr}.card-buyer,.card-info,.card-settlement{padding:12px 0 0;border-top:1px solid #edf1ef;border-left:0}.order-card__checklist,.detail-checklist-entry{align-items:flex-start;flex-direction:column}.order-card__checklist :deep(.ant-btn),.detail-checklist-entry :deep(.ant-btn){width:100%}}
</style>
