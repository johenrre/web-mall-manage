        ...
        <article v-for="record in orders" :key="record.id" class="order-card" :class="`is-${record.status}`">
          ...
          <div class="order-card__body">
            <section class="card-design">
              <BraceletPreview :pattern="record.pattern" :material-map="materialMapOf(record)" :size="82" />
              <div><span class="block-label">定制设计</span><h3>{{ record.design_name||'已删除的设计' }}</h3><p>{{ beadCount(record) }} 颗/件 · {{ materialsOf(record).length }} 种珠材 · <b>{{ record.quantity||1 }} 件</b></p></div>
            </section>
            ...
            <section class="card-fulfillment"><span class="block-label">制作与配送</span><p><i>制作</i>{{ optionText('production',record.production_method) }}</p><p><i>包装</i>{{ optionText('packaging',record.packaging_method) }}</p><p><i>配送</i>{{ optionText('express',record.express_method) }}</p></section>
          ...
          <footer class="order-card__materials"><span class="material-title">备料</span><div class="material-tags"><span v-for="material in materialsOf(record).slice(0,5)" :key="material.id"><img v-if="material.image" :src="resolveMedia(material.image)"/><i v-else></i>{{ material.name }} <b>×{{ requiredCount(material,record) }}</b></span><small v-if="materialsOf(record).length>5">还有 {{ materialsOf(record).length-5 }} 种</small></div><button @click="openDetail(record)">查看完整清单 <RightOutlined /></button></footer>
        </article>
      </div>
    ...
    <a-drawer v-model:open="detailOpen" title="订单详情" placement="right" :width="900" @after-open-change="drawerChanged">
      ...
      <div v-if="selected" class="detail-stack">
        ...
        <a-descriptions title="订单与收货" bordered :column="2" size="small">
          ...
          <a-descriptions-item label="购买数量">{{ selected.quantity||1 }} 件</a-descriptions-item>
        ...
        <a-descriptions title="金额与支付" bordered :column="3" size="small">
          <a-descriptions-item label="设计单价">{{ money(selected.unit_price) }}</a-descriptions-item>
        ...
        <section class="design-panel">
          ...
          <div class="design-copy"><span class="eyebrow">DESIGN DETAILS</span><h3>{{ selected.design_name||'已删除的设计' }}</h3><p>{{ selected.mode==='necklace'?'项链':'手链' }} · {{ beadCount(selected) }} 颗/件 · 共 {{ selected.quantity||1 }} 件</p><div class="chips"><span>制作：{{ optionText('production',selected.production_method) }}</span><span>包装：{{ optionText('packaging',selected.packaging_method) }}</span><span>配送：{{ optionText('express',selected.express_method) }}</span><span>绳色：{{ selected.rope_color||'默认' }}</span></div><small class="snapshot-note">{{ selected.snapshot_source==='order'?'已按下单时数据固化':'历史订单，按当前素材数据展示' }}</small></div>
        </section>
        ...
        <section class="material-panel">
          ...
          <a-table row-key="id" size="small" :columns="materialColumns" :data-source="selectedMaterials" :pagination="false" :scroll="{x:650}">
            <template #bodyCell="{column,record}">
              ...
              <template v-else-if="column.key==='per_item'">{{ record.count }} 颗</template>
              <template v-else-if="column.key==='required'"><b class="required-count">{{ requiredCount(record,selected) }} 颗</b></template>
...
import { designMaterials,type DesignMaterial } from '@/utils/design'

const props=withDefaults(defineProps<{aftersales?:boolean}>(),{aftersales:false})
const materialColumns=[{title:'珠材',key:'material',width:260},{title:'尺寸',key:'size',width:90},{title:'每件',key:'per_item',width:80},{title:'本单需备',key:'required',width:100},{title:'下单时单价',key:'price',width:110}]
...
const totalRequiredBeads=computed(()=>selectedMaterials.value.reduce((sum,material)=>sum+requiredCount(material,selected.value),0))
const materialMap=computed(()=>{
  const stored=selected.value?.material_map
  if(stored&&typeof stored==='object'&&!Array.isArray(stored))return stored
  return Object.fromEntries((selected.value?.bead_details||[]).flatMap((bead:any)=>[[String(bead.id).replace(/^dynamic_/,''),bead],[String(bead.id),bead]]))
})
...
function materialsOf(record:any):DesignMaterial[]{if(!record)return[];const stored=record.material_summary;if(Array.isArray(stored))return stored.map((item:any)=>({...item,id:String(item.id),image:String(item.image||item.imageUrl||''),size:Number(item.size||item.mm||0),price:Number(item.price||0),count:Number(item.count||0),subtotal:Number(item.subtotal||0),missing:Boolean(item.missing)}));return designMaterials(record)}
function beadCount(record:any){return Array.isArray(record?.pattern)?record.pattern.length:Array.isArray(record?.bead_details)?record.bead_details.length:0}
function requiredCount(material:DesignMaterial,record:any){return Number(material.count||0)*Math.max(1,Number(record?.quantity||1))}
function materialMapOf(record:any){const stored=record?.material_map;if(stored&&typeof stored==='object'&&!Array.isArray(stored))return stored;return Object.fromEntries((record?.bead_details||[]).flatMap((bead:any)=>[[String(bead.id).replace(/^dynamic_/,''),bead],[String(bead.id),bead]]))}
...
async function copyMaterialList(){if(!selected.value)return;const lines=[`订单：${selected.value.order_no}`,`设计：${selected.value.design_name||'未命名设计'} × ${selected.value.quantity||1} 件`,...selectedMaterials.value.map(material=>`${material.name}${material.size?`（${material.size}mm）`:''} × ${requiredCount(material,selected.value)} 颗`)];try{await navigator.clipboard.writeText(lines.join('\n'));message.success('备料清单已复制')}catch{message.error('复制失败，请手动选择清单内容')}}
