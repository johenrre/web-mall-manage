<template>
  <div class="page-shell">
    ...
    <div v-if="!aftersales && (orders.length || selectedOrderCount)" class="order-batch-toolbar">
      <div class="order-batch-toolbar__selection">
        <a-checkbox :checked="allPageSelected" :indeterminate="pageSelectionIndeterminate" @change="togglePageSelection">全选当前页</a-checkbox>
        <span>已选 <b>{{ selectedOrderCount }}</b> 笔订单</span>
        <a-button v-if="selectedOrderCount" type="text" size="small" @click="clearOrderSelection">清空选择</a-button>
      </div>
      <a-button type="primary" :disabled="!selectedOrderCount" @click="exportSelectedOrders"><DownloadOutlined /> 导出订单表格</a-button>
    </div>

    <a-spin :spinning="loading">
      <div v-if="orders.length" class="order-list">
        <article v-for="record in orders" :key="record.id" class="order-card" :class="[`is-${record.status}`,{'is-selected':isOrderSelected(record)}]">
          <header class="order-card__header">
            <div class="order-identity"><a-checkbox v-if="!aftersales" class="order-select" :checked="isOrderSelected(record)" :aria-label="`选择订单 ${record.order_no}`" @click.stop @change="toggleOrderSelection(record,$event)"/><a class="order-link mono" @click="openDetail(record)">{{ record.order_no }}</a><span>ID {{ record.id }}</span><span>{{ dateTime(record.created_at) }}</span><span v-if="record.express_no" class="order-identity__tracking" :title="`${record.express_company||'快递'} ${record.express_no}`">{{ record.express_company||'快递' }} · <b class="mono">{{ record.express_no }}</b></span></div>
          ...
          <div class="order-card__body">
            ...
            <section class="card-info"><div class="card-info__heading"><span class="block-label">收件信息</span><div v-if="!aftersales" class="card-info__actions"><a-button type="text" size="small" class="card-info__copy" :disabled="!hasShippingInfo(record)" aria-label="复制收件信息" title="复制收件人、手机号和收货地址" @click.stop="copyShippingInfo(record)"><CopyOutlined /> 复制</a-button><a-button type="text" size="small" class="card-info__edit" :disabled="!canEditShippingInfo(record)" aria-label="修改收件信息" :title="canEditShippingInfo(record)?'修改当前订单的收件信息':'订单已发货或结束，不能修改'" @click.stop="openShippingInfo(record)"><EditOutlined /> 修改</a-button></div></div><h4>{{ record.consignee||'未填写' }} <small class="mono">{{ record.phone||'—' }}</small></h4><p :title="record.address">{{ record.address||'未填写收货地址' }}</p></section>
    ...
    <a-modal v-model:open="shippingInfoOpen" title="修改收件信息" ok-text="保存修改" :z-index="1200" :confirm-loading="saving" @ok="saveShippingInfo"><a-alert type="info" show-icon :message="`订单 ${shippingInfoTarget?.order_no||''}。这里只修改当前订单，不会同步修改用户地址簿。`" class="modal-alert"/><a-form layout="vertical"><a-form-item label="收件人" required><a-input v-model:value="shippingInfoForm.consignee" :maxlength="50" show-count placeholder="请输入收件人姓名" /></a-form-item><a-form-item label="手机号" required><a-input v-model:value="shippingInfoForm.phone" :maxlength="24" placeholder="请输入联系电话" /></a-form-item><a-form-item label="详细地址" required><a-textarea v-model:value="shippingInfoForm.address" :auto-size="{minRows:3,maxRows:6}" :maxlength="300" show-count placeholder="请输入省、市、区和详细地址" /></a-form-item></a-form></a-modal>
...
import { ClearOutlined,CopyOutlined,DownloadOutlined,EditOutlined,FlagFilled,FlagOutlined,MoreOutlined,ReloadOutlined,RightOutlined,SyncOutlined,UnorderedListOutlined } from '@ant-design/icons-vue'
...
import { designMaterials,designSequence as patternSequence,type DesignMaterial } from '@/utils/design'
...
const selected=ref<any>(),detailOpen=ref(false),shipOpen=ref(false),shippingInfoOpen=ref(false),reviewOpen=ref(false),internalNoteOpen=ref(false),checklistOpen=ref(false),braceletZoomOpen=ref(false)
const internalNoteTarget=ref<any>(),internalNoteForm=reactive<{note:string;flag:InternalFlagColor}>({note:'',flag:''})
const shippingInfoTarget=ref<any>(),shippingInfoForm=reactive({consignee:'',phone:'',address:''})
...
const selectedOrders=computed(()=>orders.value.filter(record=>selectedOrderIds.value.includes(orderKey(record))))
const selectedOrderCount=computed(()=>selectedOrderIds.value.length)
const allPageSelected=computed(()=>orders.value.length>0&&selectedOrderCount.value===orders.value.length)
const pageSelectionIndeterminate=computed(()=>{
  return selectedOrderCount.value>0&&selectedOrderCount.value<orders.value.length
})

async function load(){clearOrderSelection();loading.value=true;try{const data:any=await get('/api/order/list',{page:page.value,pageSize:pageSize.value,status:status.value,keyword:keyword.value.trim(),internal_flag:internalFlag.value,admin:true});orders.value=listFrom(data);total.value=totalFrom(data,orders.value.length);statusCounts.value=data?.status_counts||{}}catch(e){message.error(errorMessage(e))}finally{loading.value=false}}
function search(){clearOrderSelection();page.value=1;void load()} function setStatus(value:string){clearOrderSelection();status.value=value;page.value=1;void load()} function setInternalFlag(value:InternalFlagColor){clearOrderSelection();internalFlag.value=value;page.value=1;void load()} function onPageChange(nextPage:number,nextSize:number){page.value=nextSize!==pageSize.value?1:nextPage;pageSize.value=nextSize;void load()}
function filterCount(value:string){return statusCounts.value[value||'all']||0}
function orderKey(record:any){return String(record?.id??record?.order_no??'')}
function isOrderSelected(record:any){return selectedOrderIds.value.includes(orderKey(record))}
function toggleOrderSelection(record:any,event:any){const key=orderKey(record);if(!key)return;selectedOrderIds.value=Boolean(event?.target?.checked)?[...selectedOrderIds.value,key]:selectedOrderIds.value.filter(id=>id!==key)}
function togglePageSelection(event:any){selectedOrderIds.value=Boolean(event?.target?.checked)?orders.value.map(orderKey).filter(Boolean):[]}
...
function materialsOfItem(item:any):DesignMaterial[]{
  const summary=itemSnapshot(item).material_summary
  if(!Array.isArray(summary)||!summary.length)return designMaterials(itemSnapshot(item))
  return summary.map(source=>{
    ...
    return {...source,id,name:String(source?.name||'未命名珠材'),image:String(source?.image||source?.imageUrl||''),size:Number(source?.size||source?.mm||0),price,count,subtotal:Math.round(price*count*100)/100,missing:Boolean(source?.missing)}
  })
}
function materialsOf(record:any):DesignMaterial[]{
  ...
  for(const item of itemsOf(record)){
    ...
    for(const source of materialsOfItem(item)){
      const id=String(source?.id||'').replace(/^dynamic_/,'')
      if(!id)continue
...
function exportDiyMaterials(record:any){
  return diyItemsOf(record).map((item,index)=>{
    const quantity=Math.max(1,Number(item.quantity)||1)
    const details=materialsOfItem(item).map(material=>`${material.name}${material.size?` ${material.size}mm`:''} × ${Number(material.count||0)*quantity}颗`).join('、')
    return `${item.title||`DIY 定制 ${index+1}`}：${details||'无珠材快照'}`
  }).join('\n')
}
...
function hasShippingInfo(record:any){return [record?.consignee,record?.phone,record?.address].some(value=>String(value||'').trim())}
function canEditShippingInfo(record:any){return ['pending','paid'].includes(String(record?.status||''))}
function openShippingInfo(record:any){
  if(!canEditShippingInfo(record))return message.warning('订单已发货或结束，不能修改收件信息')
  shippingInfoTarget.value=record
  Object.assign(shippingInfoForm,{consignee:String(record.consignee||''),phone:String(record.phone||''),address:String(record.address||'')})
  shippingInfoOpen.value=true
}
async function saveShippingInfo(){
  const target=shippingInfoTarget.value
  if(!target)return
  const consignee=shippingInfoForm.consignee.trim(),phone=shippingInfoForm.phone.trim(),address=shippingInfoForm.address.trim()
  if(!consignee)return message.warning('请填写收件人')
  if(!/^[+()0-9\s-]{5,24}$/.test(phone))return message.warning('请填写正确的联系电话')
  if(!address)return message.warning('请填写详细收货地址')
  saving.value=true
  try{
    const result:any=await post('/api/order/update_shipping_info',{order_id:target.id,consignee,phone,address})
    Object.assign(target,{consignee:result?.consignee||consignee,phone:result?.phone||phone,address:result?.address||address})
    if(selected.value?.id===target.id)Object.assign(selected.value,target)
    shippingInfoOpen.value=false
    message.success('收件信息已更新')
    await load()
  }catch(e){message.error(errorMessage(e))}finally{saving.value=false}
}
async function writeClipboard(text:string){
  if(navigator.clipboard?.writeText){try{await navigator.clipboard.writeText(text);return}catch{/* 非 HTTPS 或权限被拒时使用兼容方式 */}}
...
async function copyShippingInfo(record:any){
  const consignee=String(record?.consignee||'').trim()
  const phone=String(record?.phone||'').trim()
  const address=String(record?.address||'').trim()
  if(!consignee&&!phone&&!address)return message.warning('该订单没有可复制的收件信息')
  const lines=[`收件人：${consignee||'未填写'}`,`手机号：${phone||'未填写'}`,`收货地址：${address||'未填写'}`]
  try{await writeClipboard(lines.join('\n'));message.success('收件信息已复制')}catch{message.error('复制失败，请手动选择收件信息')}
}
...
function exportTimeStamp(){const d=new Date();const pad=(value:number)=>String(value).padStart(2,'0');return `${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`}
function exportSelectedOrders(){
  const records=selectedOrders.value
  if(!records.length)return message.warning('请先勾选要导出的订单')
  const headers=['订单ID','订单号','下单时间','订单状态','售后状态','下单用户','用户绑定手机','收件人','收件手机号','收货地址','商品名称','商品类型','商品明细','DIY 珠材明细','DIY 珠材总颗数','商品总件数','商品金额','选项与运费','优惠抵扣','实付金额','制作方式','包装方式','绳线','配送方式','贺卡','贺卡内容','买家备注','支付时间','快递公司','快递单号','发货时间','微信发货同步','内部彩旗','内部备注']
  const rows=records.map(record=>[
    record.id,record.order_no,dateTime(record.created_at),orderStatus[record.status]?.text||record.status||'',record.refund_status?(refundMap[record.refund_status]?.text||record.refund_status):'',record.user?.nickname||record.user?.username||`用户 ${record.user_id}`,record.user?.phone||'',record.consignee||'',record.phone||'',record.address||'',itemsOf(record).map(item=>item.title||'未命名商品').join('\n'),[...new Set(itemsOf(record).map(itemTypeLabel))].join('、'),itemsOf(record).map(item=>`[${itemTypeLabel(item)}] ${item.title||'未命名商品'} × ${item.quantity||1}`).join('\n'),exportDiyMaterials(record),materialsOf(record).reduce((sum,material)=>sum+Number(material.count||0),0),totalItemQuantity(record),Number(goodsAmount(record).toFixed(2)),Number(record.extra_fee||0).toFixed(2),(Number(record.points_discount||0)+Number(record.coupon_discount||0)).toFixed(2),Number(record.total_price||0).toFixed(2),optionText(record,'production',record.production_method),optionText(record,'packaging',record.packaging_method),optionText(record,'rope',record.rope_color),optionText(record,'express',record.express_method),record.greeting_card&&record.greeting_card!=='none'?optionText(record,'greeting',record.greeting_card):'',record.greeting_message||'',record.remark||'',dateTime(record.pay_time||record.paid_time),record.express_company||'',record.express_no||'',dateTime(record.ship_time),shippingSyncText(record.wechat_shipping_status),record.internal_flag?flagMeta(record.internal_flag).label:'',record.internal_note||'',
  ])
  const csv=[headers,...rows].map(row=>row.map(exportText).join(',')).join('\r\n')
  const blob=new Blob(['\uFEFF',csv],{type:'text/csv;charset=utf-8'})
  const url=URL.createObjectURL(blob)
  const link=document.createElement('a')
  link.href=url
  link.download=`订单信息-${exportTimeStamp()}.csv`
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
  URL.revokeObjectURL(url)
  message.success(`已导出 ${records.length} 笔订单`)
}
async function copyMaterialList(){if(!selected.value)return;const greetingLines=selected.value.greeting_card&&selected.value.greeting_card!=='none'?[`贺卡：${optionText(selected.value,'greeting',selected.value.greeting_card)}`,`贺卡内容：${selected.value.greeting_message||'未填写'}`]:[];const lines=[`订单：${selected.value.order_no}`,...greetingLines,'商品：',...itemsOf(selected.value).map(item=>`- [${itemTypeLabel(item)}] ${item.title||'未命名商品'} × ${item.quantity||1}`),'DIY 备料：',...selectedMaterials.value.map(material=>`- ${material.name}${material.size?`（${material.size}mm）`:''} × ${material.count} 颗`)];try{await writeClipboard(lines.join('\n'));message.success('备料清单已复制')}catch{message.error('复制失败，请手动选择清单内容')}}
...
@media(max-width:760px){.order-batch-toolbar{align-items:stretch;flex-direction:column}.order-batch-toolbar>:deep(.ant-btn-primary){width:100%}}
</style>
