<template>
  <div class="page-shell">
    <PageHeader :title="aftersales ? '售后管理' : '订单管理'" :description="aftersales ? '集中审核售后申请并处理退货退款' : '查看订单备料清单，完成制作、发货与履约'">
      <a-input-search v-model:value="keyword" allow-clear placeholder="订单号 / 用户 / 收件人 / 手机号 / 快递单号" style="width:360px" @search="search" />
      ...
      <div v-if="orders.length" class="order-list">
        <article v-for="record in orders" :key="record.id" class="order-card" :class="`is-${record.status}`">
          ...
          <div class="order-card__body">
            ...
            <section class="card-fulfillment"><span class="block-label">制作与配送</span><p><i>制作</i>{{ optionText('production',record.production_method) }}</p><p><i>包装</i>{{ optionText('packaging',record.packaging_method) }}</p><p><i>绳线</i>{{ record.rope_color||'默认' }}</p><p><i>配送</i>{{ optionText('express',record.express_method) }}</p><p v-if="record.greeting_card"><i>贺卡</i>{{ optionText('greeting',record.greeting_card) }}</p><template v-if="record.express_no"><p><i>快递</i>{{ record.express_company||'未填写' }}</p><p class="card-tracking" :title="record.express_no"><i>单号</i><span class="mono">{{ record.express_no }}</span></p></template></section>
          ...
          <footer class="order-card__checklist">
            ...
            <a-button type="primary" @click="openChecklist(record)">查看完整清单 <RightOutlined /></a-button>
          </footer>
        </article>
      </div>
      ...
      <div v-if="selected" class="checklist-stack">
        <section class="products-panel checklist-section">
          <div class="section-heading">
            ...
            <div class="chips"><span>制作：{{ optionText('production',selected.production_method) }}</span><span>包装：{{ optionText('packaging',selected.packaging_method) }}</span><span>绳线：{{ selected.rope_color||'默认' }}</span><span>配送：{{ optionText('express',selected.express_method) }}</span><span v-if="selected.greeting_card">贺卡：{{ optionText('greeting',selected.greeting_card) }}</span></div>
          </div>
        ...
        <section v-for="item in diyItemsOf(selected)" :key="`sequence:${item.ref_id}`" class="sequence-panel checklist-section">
          ...
          <div v-if="itemSequence(item).length" class="sequence-scroll">
            <div v-for="entry in itemSequence(item)" :key="`${entry.index}-${entry.id}`" class="sequence-item" :title="entry.name">
...
import { ClearOutlined,CopyOutlined,MoreOutlined,ReloadOutlined,RightOutlined,SyncOutlined,UnorderedListOutlined } from '@ant-design/icons-vue'
...
const selected=ref<any>(),detailOpen=ref(false),shipOpen=ref(false),reviewOpen=ref(false),checklistOpen=ref(false),braceletZoomOpen=ref(false)
...
@media(max-width:1100px){.order-card__body{grid-template-columns:minmax(250px,1.2fr) minmax(210px,1fr) 145px}.card-fulfillment{display:none}.order-identity__tracking{display:inline-flex;min-width:0;max-width:260px;color:#53746a!important}.order-identity__tracking b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
@media(max-width:760px){.order-card__header,.order-identity{align-items:flex-start}.order-identity{flex-direction:column;gap:2px}.order-card__body{grid-template-columns:1fr}.card-info,.card-settlement{padding:12px 0 0;border-top:1px solid #edf1ef;border-left:0}.order-card__checklist,.detail-checklist-entry{align-items:flex-start;flex-direction:column}.order-card__checklist :deep(.ant-btn),.detail-checklist-entry :deep(.ant-btn){width:100%}}
</style>
