...
async function save(){if(groups.value.some(x=>!x.title.trim()||!x.options.length))return message.warning('每个分组都需要标题和至少一个选项');for(const group of groups.value){const optionCodes=group.options.map(x=>x.optionCode.trim());if(group.options.some(x=>!x.title.trim()||!x.optionCode.trim()))return message.warning(`“${group.title}”存在空的名称或编码`);if(new Set(optionCodes).size!==optionCodes.length)return message.warning(`“${group.title}”的选项编码不能重复`);const enabledOptions=group.options.filter(x=>x.enabled);if(!enabledOptions.length)return message.warning(`“${group.title}”至少需要启用一个选项`);if(!enabledOptions.some(x=>x.optionCode===group.selectedOptionCode))group.selectedOptionCode=enabledOptions[0]!.optionCode}saving.value=true;try{const payload=groups.value.map(({uid:_,options,...group})=>({...group,options:options.map(({uid:__,...option})=>({...option,amount:option.isFree?0:Number(option.amount||0),enabled:Boolean(option.enabled)}))}));await post('/api/admin/checkout_options_update',{value:JSON.stringify(payload)});message.success('结算选项已保存并发布')}catch(e){message.error(errorMessage(e))}finally{saving.value=false}}
onMounted(load)
</script>
